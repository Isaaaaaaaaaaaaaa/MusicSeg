...
USE_BIGVGAN="${USE_BIGVGAN:-0}"
BIGVGAN_CWD="${BIGVGAN_CWD:-$SONGFORMDB_DIR/utils/HX}"
BIGVGAN_MATCH="${BIGVGAN_MATCH:-}"
BIGVGAN_LIMIT="${BIGVGAN_LIMIT:-0}"
...
if [[ ! -f "$HX_JSONL" ]]; then
  echo "missing HX_JSONL: $HX_JSONL"
  exit 1
fi
...
if [[ "$USE_BIGVGAN" == "1" ]]; then
  mkdir -p "$HX_AUDIO_DIR"
  export MATCH="$BIGVGAN_MATCH"
  export LIMIT="$BIGVGAN_LIMIT"
  if [[ "$USE_CUDA_KERNEL" == "1" ]]; then
    export USE_CUDA_KERNEL=1
  else
    unset USE_CUDA_KERNEL
  fi
  (cd "$BIGVGAN_CWD" && bash inference_e2e.sh)
fi
